...
export async function PATCH(
    ...
    try {
        ...
        const validation = updateStatusSchema.safeParse(json);
        ...
        const existing = await prisma.scheduleStatus.findFirst({
            where: { id, tenantId: auth.tenantId },
        });

        if (!existing) {
            return NextResponse.json({ error: 'Schedule Status not found' }, { status: 404 });
        }

        const updated = await prisma.scheduleStatus.update({
            where: { id },
            data: {
                ...data,
                isCanceled: data.isCanceled !== undefined ? data.isCanceled : existing.isCanceled,
            }
        });
...
export async function DELETE(
    ...
    try {
        ...
        try {
            // Attempt hard delete
            const deleted = await prisma.scheduleStatus.delete({
                where: { id },
            });
        ...
        if (isSoftDelete) {
            ...
            const deactivated = await prisma.scheduleStatus.update({
                where: { id },
                data: { isActive: false },
            });
