 ...
 */
export function DayView({ schedules, peopleLevel = 0, employees = [], vacations = [] }: DayViewProps) {
    ...
    return (
        <div>
            {/* People level 2: summary bar */}
            {peopleLevel >= 2 && employees.length > 0 && (
                <div className="flex items-center gap-3 px-4 py-1.5 bg-gray-50 border-b text-xs">
                    {(() => {
                        const vacCount = vacations.length;
                        const schedEmps = new Set(
                            schedules.flatMap(s => s.assignments?.map((a: any) => a.employee?.id) || [])
                        ).size;
                        const avail = Math.max(0, employees.length - vacCount - schedEmps);
                        return (
                            <>
                                <span className="text-green-700 font-medium">Available: {avail}</span>
                                {vacCount > 0 && <span className="text-amber-600 font-medium">Vacation: {vacCount}</span>}
                                <span className="text-indigo-600 font-medium">Scheduled: {schedEmps}</span>
                            </>
                        );
                    })()}
                </div>
